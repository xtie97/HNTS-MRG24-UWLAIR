...
class SegResEncoder(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(blocks_down)):
            ...
            level["spatial_gate"] = CBAM(input_channel=filters)
            ...
            if i < len(blocks_down) - 1:
                ...
                level["maxpool_mask"] = nn.MaxPool3d(
                    kernel_size=kernel_size, stride=stride, padding=padding
                )
                level["avgpool_mask"] = nn.AvgPool3d(
                    kernel_size=kernel_size, stride=stride, padding=padding
                )
            else:
                level["downsample"] = nn.Identity()
                level["maxpool_mask"] = nn.Identity()
                level["avgpool_mask"] = nn.Identity()
    ...
    def _forward(self, x: torch.Tensor) -> list[torch.Tensor]:
        mask1 = x[:, -2:-1, ...]  # preRT class 1 mask
        mask2 = x[:, -1:, ...]  # preRT class 2 mask
        ...
        for level in self.layers:
            x = level["blocks"](x)
            mask_all = torch.cat((mask1_max, mask1_avg, mask2_max, mask2_avg), dim=1)
            x = level["spatial_gate"](x, mask_all)

            outputs.append(x)

            # downsample and pool the masks
            x = level["downsample"](x)
            # outputs_mask.append(mask_all)
            mask1_max = level["maxpool_mask"](mask1_max)
            mask1_avg = level["avgpool_mask"](mask1_avg)
            mask2_max = level["maxpool_mask"](mask2_max)
            mask2_avg = level["avgpool_mask"](mask2_avg)
...
class MASegResNetDS(nn.Module):
    """
    Mask-aware SegResNet (incorporate prior GTV masks)
    Adapted from SegResNetDS in Monai
    Args:
        spatial_dims: spatial dimension of the input data. Defaults to 3.
        init_filters: number of output channels for initial convolution layer. Defaults to 32.
        in_channels: number of input channels for the network. Defaults to 1.
    ...
    def _forward(
        ...
        for level in self.up_layers:
            x = level["upsample"](x)
            x += x_down.pop(0)  # norm, act, conv, norm, act, conv
            # ---------------------------

            # normal segresnet block
            x = level["blocks"](x)
