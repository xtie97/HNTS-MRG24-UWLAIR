...
# use wandb for monitoring performance (optional)
use_wandb = False
if use_wandb:
...
class DataTransformBuilder:
    ...
    def get_resample_transforms(self, resample_label=True, crop_foreground=True):
        ...
        for extra_key in extra_keys:
            ...
            elif "label" in extra_key:  # nearest interpolation for labels
                ts.append(
                    ResampleToMatchd(
                        keys=extra_key,
                        key_dst=self.image_key,
...
class Segmenter:
    def __init__(
        ...
        model = get_network(in_channels, n_class)
    ...
    def initialize_wandb(self, config):
        ...
        run_wandb = wandb.init(
            project="task1_segresnet_midRT",  # dir=os.getenv("WANDB_DIR", "cache/wandb"),
            config={
    ...
    def train(self):
        ...
        # train_files = [f for f in train_files if "midRT_0" in f['image']] # only consider train files with midRT

        # only consider validation files with midRT
        validation_files = [
            f for f in validation_files if "midRT_0" in f["image"]
        ]  # only consider validation files with midRT
        ...
        best_metric = best_metric_class1 = best_metric_class2 = -1
        pre_loop_time = time.time()

        tot_epochs = config["num_epochs"]
        for epoch in range(tot_epochs):
            ...
            # validate every num_epochs_per_validation epochs (defaults to 1, every epoch) after 200 epochs
            if (
                (
                    epoch >= 200
                    and (epoch + 1) % config["num_epochs_per_validation"] == 0
                )
                or (epoch >= 150 and epoch < 200 and (epoch + 1) % 2 == 0)
                or epoch == 0
                or epoch == 10
                or epoch == 100
            ):
    ...
    def validate(self, testing_files=None):
        ...
        testing_files = [
            f for f in testing_files if "midRT_0" in f["image"]
        ]  # only consider validation files with midRT
    ...
    def get_cache_rate(self, train_cases=0, validation_cases=0, prioritise_train=True):
        ...
        avail_memory = 0.001  # GB # don't cache the data by default
