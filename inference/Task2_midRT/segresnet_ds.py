    ...
    def __init__(
        ...
        for i in range(len(blocks_down)):
            ...
            if i < len(blocks_down) - 1:
                ...
                level["maxpool_mask"] = nn.MaxPool3d(kernel_size=kernel_size, stride=stride, padding=padding)
                level["avgpool_mask"] = nn.AvgPool3d(kernel_size=kernel_size, stride=stride, padding=padding)
            else:
